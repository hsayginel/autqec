...
class qec_code_ZX_dualities_from_magma_with_intersection:
    ...
    def preprocess_H(self):
        n = self.n
        H_rref, _, transform_rows, transform_cols = reduced_row_echelon(self.H_symp)
...
class physical_circ_of_ZX_duality:
    ...
    def circ_w_pauli_correction(self):
        ...
        if self.pauli_correct_check:
            ...
            pauli_circ = pauli_correction_ZX_duality(G, LX, D, LZ).run(symp_mat,physical_circuit)
            physical_circuit = pauli_circ + physical_circuit 
        return physical_circuit

class pauli_correction_ZX_duality:
...
class logical_circ_of_ZX_duality:
    def __init__(self,H_symp,aut):
        """
